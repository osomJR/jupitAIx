...
export async function PATCH(req, context) {
  const { organizationId, memberUserId } = await context.params;
  const body = await readJsonBody(req);

  return proxyJsonToBackend({
    backendPath: `/api/v1/organizations/${encodeURIComponent(organizationId)}/members/${encodeURIComponent(memberUserId)}`,
    method: "PATCH",
    body,
  });
}
